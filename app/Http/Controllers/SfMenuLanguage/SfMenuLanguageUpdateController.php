 ...
 * Date: 16:04:32
 * Time: 2022/09/14
 * Class SfMenuLanguageUpdateController
...
/**
     * @OA\Put(
     *      path="/api/sf-menu-language/{menuLangId}/update",
     *      tags={"Sf Menu Language"},
     *      summary="Store Sf Menu Language Data",
     *      description="Store Sf Menu Language Data",
     *      @OA\Parameter(
     *          name="menuLangId",
     *          description="Sf Menu Language menuLangId",
     *          required=true,
     *          in="path",
     *          @OA\Schema(
     *              type="string"
     ...
     *          in="query",
     *          required=false,
     *          description="SfMenuLanguage languageId",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     ...
     *          in="query",
     *          required=false,
     *          description="SfMenuLanguage menuId",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     ...
     *          in="query",
     *          required=false,
     *          description="SfMenuLanguage displayMenu",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     ...
     *      )
     *
     * Returns Sf Menu Language Update status
     */

    public function __invoke(int $menuLangId,
                             SfMenuLanguageRequest $sfMenuLanguageRequest,
                             SfMenuLanguageServiceRequest $sfMenuLanguageServiceRequest,
                             SfMenuLanguageService $sfMenuLanguageService,
                             SfMenuLanguageResponse $sfMenuLanguageResponse,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $sfMenuLanguageValidated = $sfMenuLanguageRequest->validated();

        $sfMenuLanguageServiceRequest = Lazy::copyFromArray($sfMenuLanguageValidated, $sfMenuLanguageServiceRequest, Lazy::AUTOCAST);

        
        
        $result = app()->call([$sfMenuLanguageService, 'update'], ['menuLangId' => $menuLangId, 'sfMenuLanguageServiceRequest' => $sfMenuLanguageServiceRequest]);

        if ($result->status) {
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-menu-language.index', request()->query->all()));
            }
            $response->setData(Lazy::transform($result->sfMenuLanguage, $sfMenuLanguageResponse));
            $response->setMessage("Update Data Success");
            return $smartResponse->saveDataSuccess($response);
        } else {
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-menu-language.edit', array_merge(['menuLangId' => $menuLangId], request()->query->all())));
            }

 ...
 * Date: 16:04:35
 * Time: 2022/09/14
 * Class SfMicroprocessInputUpdateController
...
/**
     * @OA\Put(
     *      path="/api/sf-microprocess-input/{microprocessInputId}/update",
     *      tags={"Sf Microprocess Input"},
     *      summary="Store Sf Microprocess Input Data",
     *      description="Store Sf Microprocess Input Data",
     *      @OA\Parameter(
     ...
     *      ),
     *      @OA\Parameter(
     *          name="microprocessInputProcessId",
     *          in="query",
     *          required=true,
     *          description="SfMicroprocessInput microprocessInputProcessId",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessInputParamId",
     *          in="query",
     *          required=true,
     *          description="SfMicroprocessInput microprocessInputParamId",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessInputParamOrder",
     *          in="query",
     *          required=true,
     *          description="SfMicroprocessInput microprocessInputParamOrder",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessInputParamParentId",
     *          in="query",
     *          required=true,
     *          description="SfMicroprocessInput microprocessInputParamParentId",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessInputAllowNull",
     *          in="query",
     *          required=false,
     *          description="SfMicroprocessInput microprocessInputAllowNull",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessInputModel",
     *          in="query",
     *          required=true,
     *          description="SfMicroprocessInput microprocessInputModel",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     ...
     *      )
     *
     * Returns Sf Microprocess Input Update status
     */

    public function __invoke(int $microprocessInputId,
                             SfMicroprocessInputRequest $sfMicroprocessInputRequest,
                             SfMicroprocessInputServiceRequest $sfMicroprocessInputServiceRequest,
                             SfMicroprocessInputService $sfMicroprocessInputService,
                             SfMicroprocessInputResponse $sfMicroprocessInputResponse,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $sfMicroprocessInputValidated = $sfMicroprocessInputRequest->validated();

        $sfMicroprocessInputServiceRequest = Lazy::copyFromArray($sfMicroprocessInputValidated, $sfMicroprocessInputServiceRequest, Lazy::AUTOCAST);

        
        
        $result = app()->call([$sfMicroprocessInputService, 'update'], ['microprocessInputId' => $microprocessInputId, 'sfMicroprocessInputServiceRequest' => $sfMicroprocessInputServiceRequest]);

        if ($result->status) {
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-microprocess-input.index', request()->query->all()));
            }
            $response->setData(Lazy::transform($result->sfMicroprocessInput, $sfMicroprocessInputResponse));
            $response->setMessage("Update Data Success");
            return $smartResponse->saveDataSuccess($response);
        } else {
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-microprocess-input.edit', array_merge(['microprocessInputId' => $microprocessInputId], request()->query->all())));
            }

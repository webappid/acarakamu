 ...
 * Date: 16:04:39
 * Time: 2022/09/14
 * Class SfMicroprocessRefProcessUpdateController
...
/**
     * @OA\Put(
     *      path="/api/sf-microprocess-ref-process/{processCode}/update",
     ...
     *      )
     *
     * Returns Sf Microprocess Ref Process Update status
     */

    public function __invoke(string $processCode,
                             SfMicroprocessRefProcessRequest $sfMicroprocessRefProcessRequest,
                             ...
                             Response $response)
    {
        ...
        $result = app()->call([$sfMicroprocessRefProcessService, 'update'], ['processCode' => $processCode, 'sfMicroprocessRefProcessServiceRequest' => $sfMicroprocessRefProcessServiceRequest]);

        if ($result->status) {
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-microprocess-ref-process.index', request()->query->all()));
            }
            ...
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-microprocess-ref-process.edit', array_merge(['processCode' => $processCode], request()->query->all())));
            }

 ...
 * Date: 14:04:49
 * Time: 2021/11/06
 * Class SfMicroprocessRefProcessListController
...
class SfMicroprocessRefProcessListController
{
    /**
     * @OA\Get(
     *      path="/api/sf-microprocess-ref-process/list",
     *      tags={"Sf Microprocess Ref Process"},
     *      summary="Get Sf Microprocess Ref Process List",
     *      description="Returns Sf Microprocess Ref Process List",
     *      @OA\Parameter(
     *          name="q",
     *          description="Sf Microprocess Ref Process q",
     *          in="query",
     *          @OA\Schema(
     *              type="string"
     *          )
     ...
     *      )
     *
     * Returns Sf Microprocess Ref Process List
     */
    /**
     * @param SfMicroprocessRefProcessSearchRequest $sfMicroprocessRefProcessSearchRequest
    ...
    public function __invoke(
                             SfMicroprocessRefProcessSearchRequest $sfMicroprocessRefProcessSearchRequest,
                             SfMicroprocessRefProcessService $sfMicroprocessRefProcessService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $sfMicroprocessRefProcessSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$sfMicroprocessRefProcessService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->sfMicroprocessRefProcessList as $item) {
                    $data[] = Lazy::transform($item, new SfMicroprocessRefProcessResponse());
                }
        }

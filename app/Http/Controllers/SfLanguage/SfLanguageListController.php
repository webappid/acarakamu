 ...
 * Date: 14:04:39
 * Time: 2021/11/06
 * Class SfLanguageListController
...
class SfLanguageListController
{
    /**
     * @OA\Get(
     *      path="/api/sf-language/list",
     *      tags={"Sf Language"},
     *      summary="Get Sf Language List",
     *      description="Returns Sf Language List",
     *      @OA\Parameter(
     *          name="q",
     *          description="Sf Language q",
     *          in="query",
     *          @OA\Schema(
     *              type="string"
     *          )
     ...
     *      )
     *
     * Returns Sf Language List
     */
    /**
     * @param SfLanguageSearchRequest $sfLanguageSearchRequest
     * @param SfLanguageService $sfLanguageService
     * @param SmartResponse $smartResponse
     * @param Response $response
     * @return mixed
     */
    public function __invoke(
                             SfLanguageSearchRequest $sfLanguageSearchRequest,
                             SfLanguageService $sfLanguageService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $sfLanguageSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$sfLanguageService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->sfLanguageList as $item) {
                    $data[] = Lazy::transform($item, new SfLanguageResponse());
                }
        }

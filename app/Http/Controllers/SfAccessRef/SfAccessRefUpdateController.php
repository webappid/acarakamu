 ...
 * Date: 16:04:23
 * Time: 2022/09/14
 * Class SfAccessRefUpdateController
...
/**
     * @OA\Put(
     *      path="/api/sf-access-ref/{accessId}/update",
     *      tags={"Sf Access Ref"},
     *      summary="Store Sf Access Ref Data",
     *      description="Store Sf Access Ref Data",
     *      @OA\Parameter(
     *          name="accessId",
     ...
     *          in="query",
     *          required=false,
     *          description="SfAccessRef accessName",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     ...
     *      )
     *
     * Returns Sf Access Ref Update status
     */

    public function __invoke(int $accessId,
                             SfAccessRefRequest $sfAccessRefRequest,
                             SfAccessRefServiceRequest $sfAccessRefServiceRequest,
                             SfAccessRefService $sfAccessRefService,
                             SfAccessRefResponse $sfAccessRefResponse,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $sfAccessRefValidated = $sfAccessRefRequest->validated();

        $sfAccessRefServiceRequest = Lazy::copyFromArray($sfAccessRefValidated, $sfAccessRefServiceRequest, Lazy::AUTOCAST);

        
        
        $result = app()->call([$sfAccessRefService, 'update'], ['accessId' => $accessId, 'sfAccessRefServiceRequest' => $sfAccessRefServiceRequest]);

        if ($result->status) {
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-access-ref.index', request()->query->all()));
            }
            $response->setData(Lazy::transform($result->sfAccessRef, $sfAccessRefResponse));
            $response->setMessage("Update Data Success");
            return $smartResponse->saveDataSuccess($response);
        } else {
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-access-ref.edit', array_merge(['accessId' => $accessId], request()->query->all())));
            }

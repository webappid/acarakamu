 ...
 * Date: 16:04:23
 * Time: 2022/09/14
 * Class SfAccessRefStoreController
...
/**
     ...
     *      )
     *
     * Returns Sf Access Ref Store status
     */
    public function __invoke(SfAccessRefRequest $sfAccessRefRequest,
                             ...
                             Response $response)
    {
        ...
        $result = app()->call([$sfAccessRefService, 'store'], ['sfAccessRefServiceRequest' => $sfAccessRefServiceRequest]);
        ...
        if ($result->status) {
            ...
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-access-ref.create', request()->query->all()));
            }

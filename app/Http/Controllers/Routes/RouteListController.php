...
class RouteListController
{
    ...
    public function __invoke(
                             ...
                             Response $response)
    {
        $searchValue = $routeSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$routeService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->routeList as $item) {
                $data[] = Lazy::transform($item, new RouteResponse());
            }
        }

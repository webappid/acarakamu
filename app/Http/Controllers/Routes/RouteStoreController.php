...
/**
     ...
     *      )
     *
     * Returns Route Store status
     */
    public function __invoke(RouteRequest $routeRequest,
                             ...
                             Response $response)
    {
        ...
        $result = app()->call([$routeService, 'store'], ['routeServiceRequest' => $routeServiceRequest]);
        ...
        if ($result->status) {
            ...
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.route.create', request()->query->all()));
            }

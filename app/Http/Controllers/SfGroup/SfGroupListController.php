...
class SfGroupListController
{
     ...
     */
    /**
     * @param SfGroupSearchRequest $sfGroupSearchRequest
    ...
    public function __invoke(
                             SfGroupSearchRequest $sfGroupSearchRequest,
                             SfGroupService $sfGroupService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $sfGroupSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$sfGroupService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->sfGroupList as $item) {
                    $data[] = Lazy::transform($item, new SfGroupResponse());
                }
        }

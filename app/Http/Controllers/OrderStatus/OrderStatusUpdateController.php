...
/**
     * @OA\Put(
     *      path="/api/order-status/{orderStatusId}/update",
     *      tags={"Order Status"},
     ...
     *      )
     *
     * Returns Order Status Update status
     */
                             ...
                             Response $response)
    {
        ...
        $result = app()->call([$orderStatusService, 'update'], ['orderStatusId' => $orderStatusId, 'orderStatusServiceRequest' => $orderStatusServiceRequest]);
        ...
        if ($result->status) {
            ...
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.order-status.edit', array_merge(['orderStatusId' => $orderStatusId], request()->query->all())));
            }

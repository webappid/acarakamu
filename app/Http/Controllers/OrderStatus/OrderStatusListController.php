...
class OrderStatusListController
{
    /**
     * @OA\Get(
     *      path="/api/order-status/list",
     *      tags={"Order Status"},
     *      summary="Get Order Status List",
     *      description="Returns Order Status List",
     *      @OA\Parameter(
     *          name="q",
     *          description="Order Status q",
     *          in="query",
     *          @OA\Schema(
     *              type="string"
     *          )
     ...
     */
    /**
     * @param OrderStatusSearchRequest $orderStatusSearchRequest
    ...
    public function __invoke(
                             OrderStatusSearchRequest $orderStatusSearchRequest,
                             OrderStatusService $orderStatusService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $orderStatusSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$orderStatusService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->orderStatusList as $item) {
                    $data[] = Lazy::transform($item, new OrderStatusResponse());
                }
        }

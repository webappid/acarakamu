 ...
 * Date: 16:04:16
 * Time: 2022/09/14
 * Class OrderStatusDeleteController
...
class OrderStatusDeleteController
{
    /**
     * @OA\Delete(
     *      path="/api/order-status/{orderStatusId}/delete",
     *      tags={"Order Status"},
    ...
    public function __invoke(int $orderStatusId,
                             ...
                             Response $response)
    {
        $result = app()->call([$orderStatusService, 'delete'], compact('orderStatusId'));

        if (!request()->wantsJson()) {
            $response->setRedirect(route('lazy.order-status.index'));
        }

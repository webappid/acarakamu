 ...
 * Date: 14:04:07
 * Time: 2021/11/06
 * Class EventGalleryListController
...
class EventGalleryListController
{
     ...
     */
    /**
     * @param EventGallerySearchRequest $eventGallerySearchRequest
    ...
    public function __invoke(
                             EventGallerySearchRequest $eventGallerySearchRequest,
                             EventGalleryService $eventGalleryService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $eventGallerySearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$eventGalleryService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->eventGalleryList as $item) {
                    $data[] = Lazy::transform($item, new EventGalleryResponse());
                }
        }

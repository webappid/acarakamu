...
/**
     * @OA\Put(
     *      path="/api/sf-menu/{menuId}/update",
     *      tags={"Sf Menu"},
     ...
     *      )
     *
     * Returns Sf Menu Update status
     */

    public function __invoke(int $menuId,
                             SfMenuRequest $sfMenuRequest,
                             ...
                             Response $response)
    {
        ...
        $result = app()->call([$sfMenuService, 'update'], ['menuId' => $menuId, 'sfMenuServiceRequest' => $sfMenuServiceRequest]);
        ...
        if ($result->status) {
            ...
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-menu.edit', array_merge(['menuId' => $menuId], request()->query->all())));
            }

...
class SystemLogListController
{
     ...
     */
    /**
     * @param SystemLogSearchRequest $systemLogSearchRequest
    ...
    public function __invoke(
                             SystemLogSearchRequest $systemLogSearchRequest,
                             SystemLogService $systemLogService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $systemLogSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$systemLogService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->systemLogList as $item) {
                    $data[] = Lazy::transform($item, new SystemLogResponse());
                }
        }

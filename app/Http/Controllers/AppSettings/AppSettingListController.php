 ...
 * Date: 14:04:03
 * Time: 2021/11/06
 * Class AppSettingListController
...
class AppSettingListController
{
     ...
     */
    /**
     * @param AppSettingSearchRequest $appSettingSearchRequest
    ...
    public function __invoke(
                             AppSettingSearchRequest $appSettingSearchRequest,
                             AppSettingService $appSettingService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $appSettingSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$appSettingService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->appSettingList as $item) {
                    $data[] = Lazy::transform($item, new AppSettingResponse());
                }
        }

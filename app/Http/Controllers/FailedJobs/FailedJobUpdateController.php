 ...
 * Date: 16:04:07
 * Time: 2022/09/14
 * Class FailedJobUpdateController
...
/**
     * @OA\Put(
     *      path="/api/failed-job/{uuid}/update",
     *      tags={"Failed Job"},
     ...
     *      )
     *
     * Returns Failed Job Update status
     */

    public function __invoke(string $uuid,
                             FailedJobRequest $failedJobRequest,
                             ...
                             Response $response)
    {
        ...
        $result = app()->call([$failedJobService, 'update'], ['uuid' => $uuid, 'failedJobServiceRequest' => $failedJobServiceRequest]);
        ...
        if ($result->status) {
            ...
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.failed-job.edit', array_merge(['uuid' => $uuid], request()->query->all())));
            }

...
class FailedJobListController
{
     ...
     */
    /**
     * @param FailedJobSearchRequest $failedJobSearchRequest
    ...
    public function __invoke(
                             FailedJobSearchRequest $failedJobSearchRequest,
                             FailedJobService $failedJobService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $failedJobSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$failedJobService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->failedJobList as $item) {
                    $data[] = Lazy::transform($item, new FailedJobResponse());
                }
        }

...
/**
     * @OA\Put(
     *      path="/api/member/{memberId}/update",
     *      tags={"Member"},
                             ...
                             Response $response)
    {
        ...
        $result = app()->call([$memberService, 'update'], ['memberId' => $memberId, 'memberServiceRequest' => $memberServiceRequest]);
        ...
        if ($result->status) {
            ...
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.member.edit', array_merge(['memberId' => $memberId], request()->query->all())));
            }

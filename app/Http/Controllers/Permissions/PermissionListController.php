...
class PermissionListController
{
     ...
     */
    /**
     * @param PermissionSearchRequest $permissionSearchRequest
    ...
    public function __invoke(
                             ...
                             Response $response)
    {
        $searchValue = $permissionSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$permissionService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->permissionList as $item) {
                    $data[] = Lazy::transform($item, new PermissionResponse());
                }
        }

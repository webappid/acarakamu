 ...
 * Date: 14:04:43
 * Time: 2021/11/06
 * Class SfMicroprocessListController
...
class SfMicroprocessListController
{
    /**
     * @OA\Get(
     *      path="/api/sf-microprocess/list",
     *      tags={"Sf Microprocess"},
     *      summary="Get Sf Microprocess List",
     *      description="Returns Sf Microprocess List",
     *      @OA\Parameter(
     *          name="q",
     *          description="Sf Microprocess q",
     *          in="query",
     *          @OA\Schema(
     *              type="string"
     *          )
     ...
     *      )
     *
     * Returns Sf Microprocess List
     */
    /**
     * @param SfMicroprocessSearchRequest $sfMicroprocessSearchRequest
     * @param SfMicroprocessService $sfMicroprocessService
     * @param SmartResponse $smartResponse
     * @param Response $response
     * @return mixed
     */
    public function __invoke(
                             SfMicroprocessSearchRequest $sfMicroprocessSearchRequest,
                             SfMicroprocessService $sfMicroprocessService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $sfMicroprocessSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$sfMicroprocessService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->sfMicroprocessList as $item) {
                    $data[] = Lazy::transform($item, new SfMicroprocessResponse());
                }
        }

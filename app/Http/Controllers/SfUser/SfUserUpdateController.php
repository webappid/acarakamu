...
/**
     * @OA\Put(
     *      path="/api/sf-user/{userName}/update",
     *      tags={"Sf User"},
     *      summary="Store Sf User Data",
     *      description="Store Sf User Data",
     *      @OA\Parameter(
     *          name="userName",
     ...
     *          in="query",
     *          required=true,
     *          description="SfUser activateKey",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     ...
     *          in="query",
     *          required=true,
     *          description="SfUser dateTimeChange",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     ...
     *      )
     *
     * Returns Sf User Update status
     */

    public function __invoke(string $userName,
                             SfUserRequest $sfUserRequest,
                             SfUserServiceRequest $sfUserServiceRequest,
                             SfUserService $sfUserService,
                             SfUserResponse $sfUserResponse,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $sfUserValidated = $sfUserRequest->validated();

        $sfUserServiceRequest = Lazy::copyFromArray($sfUserValidated, $sfUserServiceRequest, Lazy::AUTOCAST);

        
        
        $result = app()->call([$sfUserService, 'update'], ['userName' => $userName, 'sfUserServiceRequest' => $sfUserServiceRequest]);

        if ($result->status) {
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-user.index', request()->query->all()));
            }
            $response->setData(Lazy::transform($result->sfUser, $sfUserResponse));
            $response->setMessage("Update Data Success");
            return $smartResponse->saveDataSuccess($response);
        } else {
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-user.edit', array_merge(['userName' => $userName], request()->query->all())));
            }

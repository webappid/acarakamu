...
class SfConfigListController
{
    /**
     * @OA\Get(
     *      path="/api/sf-config/list",
     *      tags={"Sf Config"},
     *      summary="Get Sf Config List",
     *      description="Returns Sf Config List",
     *      @OA\Parameter(
     *          name="q",
     *          description="Sf Config q",
     *          in="query",
     *          @OA\Schema(
     *              type="string"
     *          )
     ...
     *      )
     *
     * Returns Sf Config List
     */
    /**
     * @param SfConfigSearchRequest $sfConfigSearchRequest
     * @param SfConfigService $sfConfigService
     * @param SmartResponse $smartResponse
     * @param Response $response
     * @return mixed
     */
    public function __invoke(
                             SfConfigSearchRequest $sfConfigSearchRequest,
                             SfConfigService $sfConfigService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $sfConfigSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$sfConfigService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->sfConfigList as $item) {
                    $data[] = Lazy::transform($item, new SfConfigResponse());
                }
        }

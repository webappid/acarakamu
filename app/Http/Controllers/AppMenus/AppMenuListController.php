...
class AppMenuListController
{
     ...
     */
    /**
     * @param AppMenuSearchRequest $appMenuSearchRequest
    ...
    public function __invoke(
                             AppMenuSearchRequest $appMenuSearchRequest,
                             AppMenuService $appMenuService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $appMenuSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$appMenuService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->appMenuList as $item) {
                    $data[] = Lazy::transform($item, new AppMenuResponse());
                }
        }

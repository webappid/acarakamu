 ...
 */
namespace App\Http\Controllers\SfGroupModule;
 ...
 * Date: 16:04:27
 * Time: 2022/09/14
 * Class SfGroupModuleUpdateController
 * @package App\Http\Controllers\SfGroupModule
 */
class SfGroupModuleUpdateController
{
/**
     * @OA\Put(
     *      path="/api/sf-group-module/{groupModId}/update",
     *      tags={"Sf Group Module"},
     *      summary="Store Sf Group Module Data",
     *      description="Store Sf Group Module Data",
     *      @OA\Parameter(
     *          name="groupModId",
     *          description="Sf Group Module groupModId",
     *          required=true,
     *          in="path",
     *          @OA\Schema(
     *              type="string"
     ...
     *          in="query",
     *          required=true,
     *          description="SfGroupModule groupId",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     ...
     *          in="query",
     *          required=true,
     *          description="SfGroupModule moduleId",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     ...
     *          in="query",
     *          required=false,
     *          description="SfGroupModule accessId",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     ...
     *      )
     *
     * Returns Sf Group Module Update status
     */

    public function __invoke(int $groupModId,
                             SfGroupModuleRequest $sfGroupModuleRequest,
                             SfGroupModuleServiceRequest $sfGroupModuleServiceRequest,
                             SfGroupModuleService $sfGroupModuleService,
                             SfGroupModuleResponse $sfGroupModuleResponse,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $sfGroupModuleValidated = $sfGroupModuleRequest->validated();

        $sfGroupModuleServiceRequest = Lazy::copyFromArray($sfGroupModuleValidated, $sfGroupModuleServiceRequest, Lazy::AUTOCAST);

        
        
        $result = app()->call([$sfGroupModuleService, 'update'], ['groupModId' => $groupModId, 'sfGroupModuleServiceRequest' => $sfGroupModuleServiceRequest]);

        if ($result->status) {
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-group-module.index', request()->query->all()));
            }
            $response->setData(Lazy::transform($result->sfGroupModule, $sfGroupModuleResponse));
            $response->setMessage("Update Data Success");
            return $smartResponse->saveDataSuccess($response);
        } else {
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-group-module.edit', array_merge(['groupModId' => $groupModId], request()->query->all())));
            }

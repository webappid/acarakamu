 ...
 * Date: 14:04:37
 * Time: 2021/11/06
 * Class SfGroupModuleListController
...
class SfGroupModuleListController
{
    /**
     * @OA\Get(
     *      path="/api/sf-group-module/list",
     *      tags={"Sf Group Module"},
     *      summary="Get Sf Group Module List",
     *      description="Returns Sf Group Module List",
     *      @OA\Parameter(
     *          name="q",
     *          description="Sf Group Module q",
     *          in="query",
     *          @OA\Schema(
     *              type="string"
     *          )
     ...
     *      )
     *
     * Returns Sf Group Module List
     */
    /**
     * @param SfGroupModuleSearchRequest $sfGroupModuleSearchRequest
    ...
    public function __invoke(
                             SfGroupModuleSearchRequest $sfGroupModuleSearchRequest,
                             SfGroupModuleService $sfGroupModuleService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $sfGroupModuleSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$sfGroupModuleService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->sfGroupModuleList as $item) {
                    $data[] = Lazy::transform($item, new SfGroupModuleResponse());
                }
        }

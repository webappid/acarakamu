 ...
 * Date: 14:04:55
 * Time: 2021/11/06
 * Class SfUserResetPasswordHistListController
...
class SfUserResetPasswordHistListController
{
    /**
     * @OA\Get(
     *      path="/api/sf-user-reset-password-hist/list",
     ...
     *      @OA\Parameter(
     *          name="q",
     *          description="Sf User Reset Password Hist q",
     *          in="query",
     *          @OA\Schema(
     *              type="string"
     *          )
     ...
     */
    /**
     * @param SfUserResetPasswordHistSearchRequest $sfUserResetPasswordHistSearchRequest
     * @param SfUserResetPasswordHistService $sfUserResetPasswordHistService
     * @param SmartResponse $smartResponse
     * @param Response $response
     * @return mixed
     */
    public function __invoke(
                             SfUserResetPasswordHistSearchRequest $sfUserResetPasswordHistSearchRequest,
                             SfUserResetPasswordHistService $sfUserResetPasswordHistService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $sfUserResetPasswordHistSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$sfUserResetPasswordHistService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->sfUserResetPasswordHistList as $item) {
                    $data[] = Lazy::transform($item, new SfUserResetPasswordHistResponse());
                }
        }

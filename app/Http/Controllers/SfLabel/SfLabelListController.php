...
class SfLabelListController
{
    /**
     * @OA\Get(
     *      path="/api/sf-label/list",
     *      tags={"Sf Label"},
     *      summary="Get Sf Label List",
     *      description="Returns Sf Label List",
     *      @OA\Parameter(
     *          name="q",
     *          description="Sf Label q",
     *          in="query",
     *          @OA\Schema(
     *              type="string"
     *          )
     ...
     *      )
     *
     * Returns Sf Label List
     */
    /**
     * @param SfLabelSearchRequest $sfLabelSearchRequest
    ...
    public function __invoke(
                             SfLabelSearchRequest $sfLabelSearchRequest,
                             SfLabelService $sfLabelService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $sfLabelSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$sfLabelService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->sfLabelList as $item) {
                    $data[] = Lazy::transform($item, new SfLabelResponse());
                }
        }

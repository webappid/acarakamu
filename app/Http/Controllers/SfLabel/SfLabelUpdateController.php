...
/**
     * @OA\Put(
     *      path="/api/sf-label/{labelId}/update",
     *      tags={"Sf Label"},
     *      summary="Store Sf Label Data",
     *      description="Store Sf Label Data",
     *      @OA\Parameter(
     *          name="labelId",
     *          description="Sf Label labelId",
     *          required=true,
     *          in="path",
     *          @OA\Schema(
     *              type="string"
     ...
     *      )
     *
     * Returns Sf Label Update status
     */

    public function __invoke(int $labelId,
                             SfLabelRequest $sfLabelRequest,
                             ...
                             Response $response)
    {
        ...
        $result = app()->call([$sfLabelService, 'update'], ['labelId' => $labelId, 'sfLabelServiceRequest' => $sfLabelServiceRequest]);
        ...
        if ($result->status) {
            ...
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-label.edit', array_merge(['labelId' => $labelId], request()->query->all())));
            }

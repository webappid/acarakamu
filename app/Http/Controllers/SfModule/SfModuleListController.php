...
class SfModuleListController
{
     ...
     */
    /**
     * @param SfModuleSearchRequest $sfModuleSearchRequest
     * @param SfModuleService $sfModuleService
     * @param SmartResponse $smartResponse
     * @param Response $response
     * @return mixed
     */
    public function __invoke(
                             SfModuleSearchRequest $sfModuleSearchRequest,
                             SfModuleService $sfModuleService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $sfModuleSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$sfModuleService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->sfModuleList as $item) {
                    $data[] = Lazy::transform($item, new SfModuleResponse());
                }
        }

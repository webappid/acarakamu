 ...
 * Date: 14:04:46
 * Time: 2021/11/06
 * Class SfMicroprocessProcessListController
...
class SfMicroprocessProcessListController
{
     ...
     */
    /**
     * @param SfMicroprocessProcessSearchRequest $sfMicroprocessProcessSearchRequest
    ...
    public function __invoke(
                             SfMicroprocessProcessSearchRequest $sfMicroprocessProcessSearchRequest,
                             SfMicroprocessProcessService $sfMicroprocessProcessService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $sfMicroprocessProcessSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$sfMicroprocessProcessService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->sfMicroprocessProcessList as $item) {
                    $data[] = Lazy::transform($item, new SfMicroprocessProcessResponse());
                }
        }

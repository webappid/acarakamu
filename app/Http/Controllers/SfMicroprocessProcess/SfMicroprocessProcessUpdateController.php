 ...
 * Date: 16:04:36
 * Time: 2022/09/14
 * Class SfMicroprocessProcessUpdateController
...
/**
     * @OA\Put(
     *      path="/api/sf-microprocess-process/{microprocessProcessId}/update",
     *      tags={"Sf Microprocess Process"},
     *      summary="Store Sf Microprocess Process Data",
     *      description="Store Sf Microprocess Process Data",
     *      @OA\Parameter(
     ...
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessMicroprocessId",
     *          in="query",
     *          required=true,
     *          description="SfMicroprocessProcess microprocessProcessMicroprocessId",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessOrder",
     *          in="query",
     *          required=true,
     *          description="SfMicroprocessProcess microprocessProcessOrder",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessProcessId",
     *          in="query",
     *          required=false,
     *          description="SfMicroprocessProcess microprocessProcessProcessId",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessNext",
     *          in="query",
     *          required=true,
     *          description="SfMicroprocessProcess microprocessProcessNext",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessJumpProcess",
     *          in="query",
     *          required=false,
     *          description="SfMicroprocessProcess microprocessProcessJumpProcess",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessJoin",
     *          in="query",
     *          required=false,
     *          description="SfMicroprocessProcess microprocessProcessJoin",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessLinkId",
     *          in="query",
     *          required=false,
     *          description="SfMicroprocessProcess microprocessProcessLinkId",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessKeyId",
     *          in="query",
     *          required=false,
     *          description="SfMicroprocessProcess microprocessProcessKeyId",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessForeignId",
     *          in="query",
     *          required=false,
     *          description="SfMicroprocessProcess microprocessProcessForeignId",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessEmpty",
     *          in="query",
     *          required=false,
     *          description="SfMicroprocessProcess microprocessProcessEmpty",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessFalseCode",
     *          in="query",
     *          required=false,
     *          description="SfMicroprocessProcess microprocessProcessFalseCode",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessFalseMessage",
     *          in="query",
     *          required=false,
     *          description="SfMicroprocessProcess microprocessProcessFalseMessage",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessTrueCode",
     *          in="query",
     *          required=false,
     *          description="SfMicroprocessProcess microprocessProcessTrueCode",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     *      @OA\Parameter(
     *          name="microprocessProcessTrueMessage",
     *          in="query",
     *          required=false,
     *          description="SfMicroprocessProcess microprocessProcessTrueMessage",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     ...
     *      )
     *
     * Returns Sf Microprocess Process Update status
     */

    public function __invoke(int $microprocessProcessId,
                             SfMicroprocessProcessRequest $sfMicroprocessProcessRequest,
                             ...
                             SfMicroprocessProcessResponse $sfMicroprocessProcessResponse,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $sfMicroprocessProcessValidated = $sfMicroprocessProcessRequest->validated();

        $sfMicroprocessProcessServiceRequest = Lazy::copyFromArray($sfMicroprocessProcessValidated, $sfMicroprocessProcessServiceRequest, Lazy::AUTOCAST);

        
        
        $result = app()->call([$sfMicroprocessProcessService, 'update'], ['microprocessProcessId' => $microprocessProcessId, 'sfMicroprocessProcessServiceRequest' => $sfMicroprocessProcessServiceRequest]);

        if ($result->status) {
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-microprocess-process.index', request()->query->all()));
            }
            $response->setData(Lazy::transform($result->sfMicroprocessProcess, $sfMicroprocessProcessResponse));
            $response->setMessage("Update Data Success");
            return $smartResponse->saveDataSuccess($response);
        } else {
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-microprocess-process.edit', array_merge(['microprocessProcessId' => $microprocessProcessId], request()->query->all())));
            }

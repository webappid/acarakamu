 ...
 * Date: 16:04:26
 * Time: 2022/09/14
 * Class SfGroupMenuUpdateController
...
/**
     * @OA\Put(
     *      path="/api/sf-group-menu/{groupMenuId}/update",
     *      tags={"Sf Group Menu"},
     ...
     *      )
     *
     * Returns Sf Group Menu Update status
     */

    public function __invoke(int $groupMenuId,
                             SfGroupMenuRequest $sfGroupMenuRequest,
                             ...
                             Response $response)
    {
        ...
        $result = app()->call([$sfGroupMenuService, 'update'], ['groupMenuId' => $groupMenuId, 'sfGroupMenuServiceRequest' => $sfGroupMenuServiceRequest]);
        ...
        if ($result->status) {
            ...
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-group-menu.edit', array_merge(['groupMenuId' => $groupMenuId], request()->query->all())));
            }

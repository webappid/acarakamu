 ...
 * Date: 16:04:20
 * Time: 2022/09/14
 * Class RolePermissionUpdateController
...
/**
     * @OA\Put(
     *      path="/api/role-permission/{id}/update",
     ...
     *      )
     *
     * Returns Role Permission Update status
     */
                             ...
                             Response $response)
    {
        ...
        $result = app()->call([$rolePermissionService, 'update'], ['id' => $id, 'rolePermissionServiceRequest' => $rolePermissionServiceRequest]);
        ...
        if ($result->status) {
            ...
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.role-permission.edit', array_merge(['id' => $id], request()->query->all())));
            }

 ...
 * Date: 14:04:30
 * Time: 2021/11/06
 * Class RolePermissionListController
...
class RolePermissionListController
{
     ...
     */
    /**
     * @param RolePermissionSearchRequest $rolePermissionSearchRequest
    ...
    public function __invoke(
                             RolePermissionSearchRequest $rolePermissionSearchRequest,
                             RolePermissionService $rolePermissionService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $rolePermissionSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$rolePermissionService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->rolePermissionList as $item) {
                    $data[] = Lazy::transform($item, new RolePermissionResponse());
                }
        }

 ...
 * Date: 14:04:47
 * Time: 2021/11/06
 * Class SfMicroprocessRefParamListController
...
class SfMicroprocessRefParamListController
{
    /**
     * @OA\Get(
     *      path="/api/sf-microprocess-ref-param/list",
     *      tags={"Sf Microprocess Ref Param"},
     *      summary="Get Sf Microprocess Ref Param List",
     *      description="Returns Sf Microprocess Ref Param List",
     *      @OA\Parameter(
     *          name="q",
     *          description="Sf Microprocess Ref Param q",
     *          in="query",
     *          @OA\Schema(
     *              type="string"
     *          )
     ...
     *      )
     *
     * Returns Sf Microprocess Ref Param List
     */
    /**
     * @param SfMicroprocessRefParamSearchRequest $sfMicroprocessRefParamSearchRequest
     * @param SfMicroprocessRefParamService $sfMicroprocessRefParamService
     * @param SmartResponse $smartResponse
     * @param Response $response
     * @return mixed
     */
    public function __invoke(
                             SfMicroprocessRefParamSearchRequest $sfMicroprocessRefParamSearchRequest,
                             SfMicroprocessRefParamService $sfMicroprocessRefParamService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $sfMicroprocessRefParamSearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$sfMicroprocessRefParamService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->sfMicroprocessRefParamList as $item) {
                    $data[] = Lazy::transform($item, new SfMicroprocessRefParamResponse());
                }
        }

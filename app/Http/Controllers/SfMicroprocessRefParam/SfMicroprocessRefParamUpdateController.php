 ...
 * Date: 16:04:37
 * Time: 2022/09/14
 * Class SfMicroprocessRefParamUpdateController
...
/**
     * @OA\Put(
     *      path="/api/sf-microprocess-ref-param/{paramName}/update",
     ...
     *      @OA\Parameter(
     *          name="paramName",
     *          description="Sf Microprocess Ref Param paramName",
     *          required=true,
     *          in="path",
     *          @OA\Schema(
     *              type="string"
     ...
     *      )
     *
     * Returns Sf Microprocess Ref Param Update status
     */

    public function __invoke(string $paramName,
                             SfMicroprocessRefParamRequest $sfMicroprocessRefParamRequest,
                             ...
                             Response $response)
    {
        ...
        $result = app()->call([$sfMicroprocessRefParamService, 'update'], ['paramName' => $paramName, 'sfMicroprocessRefParamServiceRequest' => $sfMicroprocessRefParamServiceRequest]);
        ...
        if ($result->status) {
            ...
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.sf-microprocess-ref-param.edit', array_merge(['paramName' => $paramName], request()->query->all())));
            }

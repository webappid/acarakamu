 ...
 * Date: 16:03:58
 * Time: 2022/09/14
 * Class CategoryRefUpdateController
...
/**
     * @OA\Put(
     *      path="/api/category-ref/{categoryId}/update",
     *      tags={"Category Ref"},
     ...
     *      )
     *
     * Returns Category Ref Update status
     */

    public function __invoke(int $categoryId,
                             CategoryRefRequest $categoryRefRequest,
                             ...
                             Response $response)
    {
        ...
        $result = app()->call([$categoryRefService, 'update'], ['categoryId' => $categoryId, 'categoryRefServiceRequest' => $categoryRefServiceRequest]);
        ...
        if ($result->status) {
            ...
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.category-ref.edit', array_merge(['categoryId' => $categoryId], request()->query->all())));
            }

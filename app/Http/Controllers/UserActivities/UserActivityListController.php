...
use App\Requests\SfUserResetPasswordHistSearchRequest;
use App\Requests\SfUserSearchRequest;
use App\Requests\SystemLogSearchRequest;
use App\Requests\UserActivitySearchRequest;
 ...
 * Date: 14:04:58
 * Time: 2021/11/06
 * Class UserActivityListController
...
class UserActivityListController
{
    /**
     * @OA\Get(
     *      path="/api/user-activity/list",
     *      tags={"User Activity"},
     *      summary="Get User Activity List",
     *      description="Returns User Activity List",
     *      @OA\Parameter(
     *          name="q",
     *          description="User Activity q",
     *          in="query",
     *          @OA\Schema(
     *              type="string"
     *          )
     ...
     */
    /**
     * @param UserActivitySearchRequest $userActivitySearchRequest
    ...
    public function __invoke(
                             UserActivitySearchRequest $userActivitySearchRequest,
                             UserActivityService $userActivityService,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $searchValue = $userActivitySearchRequest->validated();
        $q = "";
        ...
        $result = app()->call([$userActivityService, 'get'], ['q' => $q]);

        $data = [];
        if($result->status){
            foreach ($result->userActivityList as $item) {
                    $data[] = Lazy::transform($item, new UserActivityResponse());
                }
        }

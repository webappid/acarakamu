...
*/
namespace App\Http\Controllers\UserActivities;
...
use App\Requests\UserActivityRequest;
...
use App\Responses\UserActivityResponse;
 ...
 * Date: 14:04:58
 * Time: 2021/11/06
 * Class UserActivityStoreController
 * @package App\Http\Controllers\UserActivities
 */
class UserActivityStoreController
{
/**
     * @OA\Post(
     *      path="/api/user-activity/store",
     *      tags={"User Activity"},
     *      summary="Store User Activity Data",
     *      description="Store User Activity Data",
     *      @OA\Parameter(
     *          name="user_id",
     *          in="query",
     *          required=false,
     *          description="UserActivity user_id",
     *          @OA\Schema(
     *              type="int"
     *          )
     *      ),
     ...
     *          in="query",
     *          required=true,
     *          description="UserActivity activity",
     *          @OA\Schema(
     *              type="string"
     *          )
     *      ),
     ...
     *      )
     *
     * Returns User Activity Store status
     */
    /**
     * @param UserActivityRequest $userActivityRequest
     * @param UserActivityServiceRequest $userActivityServiceRequest
     ...
     * @throws Exception
     */
    public function __invoke(UserActivityRequest $userActivityRequest,
                             UserActivityServiceRequest $userActivityServiceRequest,
                             UserActivityService $userActivityService,
                             UserActivityResponse $userActivityResponse,
                             SmartResponse $smartResponse,
                             Response $response)
    {
        $userActivityValidated = $userActivityRequest->validated();

        $userActivityServiceRequest = Lazy::copyFromArray($userActivityValidated, $userActivityServiceRequest, Lazy::AUTOCAST);

        
        
        $result = app()->call([$userActivityService, 'store'], ['userActivityServiceRequest' => $userActivityServiceRequest]);

        if ($result->status) {
            
            $response->setData(Lazy::transform($result->userActivity, $userActivityResponse));

 ...
 * Date: 16:04:06
 * Time: 2022/09/14
 * Class EventWishUpdateController
...
/**
     * @OA\Put(
     *      path="/api/event-wish/{wishListId}/update",
     *      tags={"Event Wish"},
     ...
     *      )
     *
     * Returns Event Wish Update status
     */

    public function __invoke(int $wishListId,
                             EventWishRequest $eventWishRequest,
                             ...
                             Response $response)
    {
        ...
        $result = app()->call([$eventWishService, 'update'], ['wishListId' => $wishListId, 'eventWishServiceRequest' => $eventWishServiceRequest]);
        ...
        if ($result->status) {
            ...
            if (!request()->wantsJson()) {
                $response->setRedirect(route('lazy.event-wish.edit', array_merge(['wishListId' => $wishListId], request()->query->all())));
            }
